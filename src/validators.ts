...
export const vehicleSpecificationSchema = z.object({
  ...
  features: z.string()
    ...
    .refine(
      (val) => val.includes("2-wheeler" ) || val.includes("4-wheeler" ),
      { message: "Features must include either '2-wheeler' or '4-wheeler'" }
    ),
});
...
export const bookingSchema = z.object({
  ...
  bookingDate: z.preprocess((arg) => new Date(arg as string), z.date()),
  returnDate: z.preprocess((arg) => new Date(arg as string), z.date()),
...
export const fleetManagementSchema = z.object({
  vehicleId: z.number().int().positive("Vehicle ID must be positive"),
  acquisitionDate: z.preprocess((arg) => new Date(arg as string), z.date()),
  depreciationRate: z
    .number()
    .int()
